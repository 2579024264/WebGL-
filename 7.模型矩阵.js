...
function main() {
    ...
    var u_ModeMatrix = gl.getUniformLocation(program, 'u_ModeMatrix');

    var Tx = 0.5; // 平移距离
    var ANGLE = 90.0; // 旋转角

    var modeMatrix = new Matrix4();
    modeMatrix.setTranslate(Tx, 0, 0);// 将模型矩阵乘以平移矩阵
    modeMatrix.rotate(ANGLE, 0, 0, 1); // 绕z轴顺时针旋转90度
    modeMatrix.scale(0.5, 0.5, 0.5); // x,y,z轴的缩放因子为0.5
    // 将模型矩阵传给顶点着色器
